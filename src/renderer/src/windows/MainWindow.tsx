...
import { G, page, card, label, primaryBtn, dangerBtn, textBtn, outlineBtn } from "../theme";
...
export function MainWindow(): JSX.Element {
  ...
  return (
    <div style={{ ...page, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div>
        <div style={{ fontSize: 18, fontWeight: 600, color: G.text }}>OpenCraft Recorder</div>
      ...
      {phase === "setup" && (
        <>
          ...
          <div style={{ ...card, padding: 14 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: G.text, marginTop: 6 }}>
          ...
          <div style={{ ...card, padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {sources.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    textAlign: "left",
                    padding: 4,
                    background: sel === s.id ? G.blueSoftBg : "#fff",
                    border: `2px solid ${sel === s.id ? G.blue : G.cardBorder}`,
                    borderRadius: 8,
                    color: G.text,
                    fontFamily: G.font,
                  }}
                >
                  ...
                  <div style={{ fontSize: 11, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: G.textSub }}>
                    {s.kind === "screen" ? "🖥 " : "🪟 "}
                    {s.name}
                  </div>
                </button>
            ...
            <button style={primaryBtn(!sel)} onClick={() => void start()} disabled={!sel}>
        ...
        <>
          ...
          <div style={{ ...card, padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span
                style={{
                  ...
                  height: 9,
                  borderRadius: "50%",
                  background: phase === "recording" ? G.red : phase === "paused" ? G.textFaint : "#C7CDD4",
                }}
              />
              <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 18, fontWeight: 700, color: G.text }}>
                {fmtClock(elapsed)}
              </span>
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                <button style={zoomBtn} onClick={zoomOut} aria-label="Zoom out">−</button>
                <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 13, minWidth: 40, textAlign: "center", color: G.text }}>
                  {zoom.toFixed(1)}×
                </span>
                <button style={zoomBtn} onClick={zoomIn} aria-label="Zoom in">+</button>
              </div>
            </div>

            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: G.textSub }}>
              <input type="checkbox" checked={auto} onChange={(e) => setAuto(e.target.checked)} style={{ accentColor: G.blue }} />
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {phase === "armed" && (
                <button style={{ ...dangerBtn, flex: 1 }} onClick={onRecord}>● Record</button>
              )}
              {phase === "recording" && (
                <button style={{ ...outlineBtn, flex: 1 }} onClick={onPause}>❚❚ Pause</button>
              )}
              {phase === "paused" && (
                <button style={{ ...dangerBtn, flex: 1 }} onClick={onResume}>● Resume</button>
              )}
              {(phase === "recording" || phase === "paused") && (
                <button style={{ ...primaryBtn(), flex: 1 }} onClick={() => void onStop()}>■ Stop</button>
              )}
            </div>

            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ ...outlineBtn, flex: 1, color: G.textSub, borderColor: G.inputBorder }} onClick={() => void toggleCamera()}>
                {camOpen ? "Hide camera bubble" : "Show camera bubble"}
              </button>
              {phase === "armed" && (
                <button style={textBtn} onClick={reset}>Cancel</button>
              )}
            </div>
          </div>
      ...
      {phase === "done" && (
        <div style={{ ...card, padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
          <span style={{ ...label, color: G.green }}>RECORDING READY</span>
          {recordedUrl && (
            <video
              src={recordedUrl}
              controls
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={{ ...primaryBtn(!recordedBlob), flex: 1 }} onClick={() => void save()} disabled={!recordedBlob}>
              Save…
            </button>
            <button style={outlineBtn} onClick={reset}>Record again</button>
          </div>
        </div>
      )}
    </div>
